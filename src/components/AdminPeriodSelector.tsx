    ...
    <div className="flex items-center gap-2">
      {Object.keys(PERIOD_DAYS).map(p => (
        <button key={p} onClick={() => { setPeriod(p); setShowDatePicker(false); }}
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${p === period ? 'bg-primary text-white shadow-md' : 'bg-white text-on-surface-variant hover:bg-surface-container-low border border-outline-variant/10'}`}
        >{p}</button>
      ))}
      <div className="relative">
        <button 
          onClick={() => { setPeriod('Personalizado'); setShowDatePicker(!showDatePicker); }}
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${period === 'Personalizado' ? 'bg-primary text-white shadow-md' : 'bg-white text-on-surface-variant hover:bg-surface-container-low border border-outline-variant/10'}`}
        >
        ...
        {showDatePicker && (
          <div className="absolute top-full mt-2 left-0 bg-white p-6 rounded-[2rem] shadow-2xl border border-outline-variant/10 z-[110] w-[320px] animate-in fade-in zoom-in-95 duration-200">
