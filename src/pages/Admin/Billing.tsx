...
export const Billing: React.FC = () => {
  ...
  const hasBillingModalOpen = !!showInvoiceDetail || showUnbilledModal || showNewBillingCustomer || showDetailedCreator;
        ...
        <div className="flex gap-2 bg-white p-1.5 rounded-2xl border border-outline-variant/10 shadow-sm w-fit">
          {tabs.map(tab => (
            <button
              ...
              className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all flex items-center gap-2 ${activeTab === tab.id
                  ...
                  <tbody className="divide-y divide-outline-variant/10">
                    {filteredInvoices.map(inv => (
                      <tr key={inv.id} className="hover:bg-surface-container-lowest transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold ${inv.status === 'Emitida'
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="pt-2 border-t border-outline-variant/10 flex items-center justify-between">
                        ...
                        <button
                          ...
                          className={`w-12 h-6 rounded-full p-1 transition-colors duration-300 outline-none ${creatorPricesIncludeTax ? 'bg-primary' : 'bg-surface-container-high'
                            }`}
                        >
                          <div
                            className={`w-4 h-4 rounded-full bg-white transition-transform duration-300 shadow-sm ${creatorPricesIncludeTax ? 'translate-x-6' : 'translate-x-0'
                              }`}
                          />
                        </button>
                      </div>
                    ...
                    {unbilledOrders.map(o => {
                      ...
                      return (
                        <div
                          ...
                          className={`bg-surface-container-lowest border p-5 rounded-[1.5rem] flex items-center justify-between hover:border-primary/50 transition-all shadow-sm cursor-pointer ${isSelected ? 'border-primary bg-primary/5 ring-1 ring-primary/20' : 'border-outline-variant/10'
                            }`}
                        >
